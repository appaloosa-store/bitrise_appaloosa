...
SIG_PATH_GROUP_KEY_CPATH=`curl -H "Content-Type: application/json" -X GET --data '{"store_id":"'"$store_id"'", "md5":"'"$md5"'", "date":"'"$date"'", "group_ids":"'"$group_ids"'", "binary_file_name": '"$binary_file_name"'}' $APPALOOSA_SERVER/$store_id/bitrise_binaries/generate_signature?api_key=$appaloosa_api_key`
